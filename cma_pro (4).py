...
def _cached_generate(prompt, model_name):
    # Errors are NOT cached (exceptions propagate), only successful answers.
    return genai.GenerativeModel(model_name).generate_content(prompt).text


def ask_ai_cached(prompt, fresh=False):
    """Same prompt = saved answer, no new API request. fresh=True forces a new one."""
    if fresh:
        return ask_ai(prompt)
    try:
        return _cached_generate(prompt, MODEL_NAME)
    except Exception as e:
        st.error(f"AI error: {e}")
        return None
...
# ---------- 2. Quick Revision Guides ----------
    ...
    fresh_guide = st.checkbox("🔄 Puthu copy venum (quota use aagum)", key="fresh_guide")
...
# ---------- 4. Study Notes & CAS ----------
    ...
    fresh_notes = st.checkbox("🔄 Puthu copy venum (quota use aagum)", key="fresh_notes")
